...
BEGIN
  -- Validate required string fields are not empty and within length limits
  ...
  VALUES (trim(p_customer_name), trim(p_customer_whatsapp), trim(p_address_street), trim(p_address_number), trim(p_address_neighborhood), trim(p_address_reference), trim(p_notes), p_payment_method, p_change_for, p_total)
